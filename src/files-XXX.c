...
/* Return 1 if we should ignore the file based on its name. */
static int
...
internal_setent (DIR **dirp, FILE **filep)
{
  enum nss_status status = NSS_STATUS_SUCCESS;

  if (*dirp == NULL)
    {
      *dirp = opendir (DATAFILE ".d");
    ...
    {
      /* Close file handle for files and rewind directory */
      if (*filep != NULL)
	{
	  fclose (*filep);
	  *filep = NULL;
...
enum nss_status
CONCAT(_nss_filesd_set,ENTNAME) (int stayopen)
{
  enum nss_status status;

  __libc_lock_lock (lock);

  status = internal_setent (&dirp, &filep);
...
static void
internal_endent (DIR **dirp, FILE **filep)
{
  if (*filep != NULL)
    {
      fclose (*filep);
...
/* Thread-safe, exported version of that.  */
enum nss_status
CONCAT(_nss_filesd_end,ENTNAME) (void)
{
  __libc_lock_lock (lock);

  internal_endent (&dirp, &filep);
...
static enum nss_status
internal_getent (DIR **dirp, FILE **filep, struct STRUCTURE *result,
      ...
      if ((*filep == NULL) || feof (*filep))
	{
	  ...
	  do {
		...
		*dirp = NULL;
	    ...
	    /* Formulate the full path name and try to open it. */
	    /* XXX no snprintf! */
	    snprintf (path, sizeof (path), DATAFILE ".d/%s", ent->d_name);
	    *filep = fopen (path, "rce");
		...
		*filep = NULL;
	    ...
	    /* If the file isn't normal or a symlink, move on. */
	  ...
	  } while ((*dirp != NULL) && (*filep == NULL));
	}

      /* If we're out of data, return NOTFOUND. */
      if ((*dirp == NULL) || (*filep == NULL))
	return NSS_STATUS_NOTFOUND;

      get_contents_ret r = get_contents (data->linebuffer, linebuflen, *filep);

      if (r == gcr_error)
	{
	  /* End of file or read error. set errno for the case we find
	     nothing else anymore.  */
	  H_ERRNO_SET (HOST_NOT_FOUND);
	  /* Try next file if one exist */
	  continue;
	}
...
enum nss_status
CONCAT(_nss_filesd_get,ENTNAME_r) (struct STRUCTURE *result, char *buffer,
				  size_t buflen, int *errnop H_ERRNO_PROTO)
{
  ...
  if (status == NSS_STATUS_SUCCESS)
    status = internal_getent (&dirp, &filep, result, buffer, buflen, errnop
...
{									      \
  ...
  if (status == NSS_STATUS_SUCCESS)					      \
    {									      \
      while ((status = internal_getent (&dirp, &filep, result, buffer, buflen, errnop \
					H_ERRNO_ARG EXTRA_ARGS_VALUE))	      \
	     == NSS_STATUS_SUCCESS)					      \
	{ break_if_match }						      \
									      \
